<template>
  <bk-table :class="{selectable}">
    ...
    <tbody>
    <tr v-for="row of sortedData.slice((currentPage - 1) * selectedPageSize, currentPage * selectedPageSize)"
        :key="row"
        @click="selectRow(row, $event)"
        :class="{active: selectable && selectedRowKeys.includes(row[rowKey]), flatTop: selectedRowExistsBefore(row), flatBottom: selectedRowExistsAfter(row)}">
  ...
  <div class="bk-data-table_pagination">
    <div style="margin-right: auto">
      Page {{ currentPage }} / {{ totalPages }}
    </div>

    <bk-dropdown :items="[5, 10, 20, 50]" style="width: 70px" v-model="selectedPageSize"></bk-dropdown>
    <bk-button icon size="medium" :disabled="currentPage <= 1" style="margin-right: 5px" @click="currentPage--">
    ...
    <bk-button icon size="medium" :disabled="currentPage >= totalPages" @click="currentPage++">
      <svg xmlns="http://www.w3.org/2000/svg" style="height: 20px; width: 20px" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const totalPages = Math.ceil(sortedData.value.length / selectedPageSize.value)
